...
import {
  Send, GraduationCap, X, Loader2, Bot, User,
...
function Bubble({ msg }) {
  ...
  return (
    <div className={`flex items-end gap-2 chat-in ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 shadow-sm ${isUser
...
export default function ChatInterface() {
  ...
  const [lead, setLead] = useState({
    courseInterest: '', qualification: '', city: '', state: '', passingYear: ''
  });
  ...
  const handleSaveLead = async () => {
    try {
      ...
      setMsgs(p => [...p, {
        role: 'bot',
        message: '✅ Thank you! Your details have been saved. Our admission counselor will contact you soon. Is there anything else I can help you with?',
        createdAt: new Date()
      }]);
    } catch { /* silent */ }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 overflow-hidden">
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="flex-shrink-0 bg-gradient-to-r from-blue-600 to-purple-700 px-3 py-2.5 flex items-center gap-2 min-h-[56px]">
        ...
        {/* Title — takes all remaining space, truncates if needed */}
        ...
        {/* Close / logout button — fixed size, never shrinks */}
        <button
          onClick={logout}
          title="Close chat"
          className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-white/15 hover:bg-white/25 rounded-lg transition"
        >
      ...
      {/* ── Messages ───────────────────────────────────────────────────────── */}
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 thin-scroll">
          ...
          <div className="chat-in bg-white border border-blue-100 rounded-2xl p-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <input
                  ...
                  className={`px-3 py-2 rounded-lg border border-gray-200 bg-gray-50 text-xs text-gray-700
                    focus:outline-none focus:ring-1 focus:ring-blue-500 ${col === 2 ? 'col-span-2' : ''}`}
                />
              ))}
            </div>
            <div className="flex gap-2 mt-3">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-xs font-semibold py-2.5 rounded-lg hover:opacity-90 transition"
              >
                Submit Details
              </button>
              <button
                onClick={() => setShowLead(false)}
                className="px-3 text-gray-400 text-xs hover:text-gray-600"
              >
                Skip
              </button>
            </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1.5 scrollbar-hide">
          {QUICK.map(({ label, icon: Icon, msg }) => (
            <button
              key={label}
              onClick={() => send(msg)}
              className="flex-shrink-0 flex items-center gap-1.5 bg-blue-50 hover:bg-blue-100
                text-blue-700 text-xs font-medium px-3 py-2 rounded-full border border-blue-100 transition"
            >
      ...
      {/* ── Input bar ──────────────────────────────────────────────────────── */}
      {/*
        FIX: use safe-area padding so input stays above the iOS home bar,
        and make sure the send button never overlaps with anything.
      */}
      <form
      ...
      >
        <input
          ...
          disabled={loading}
          className="flex-1 min-w-0 bg-gray-100 rounded-full px-4 py-2.5 text-sm text-gray-800
            placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500
            focus:bg-white transition"
        />
        {/* Send button — fixed size, flex-shrink-0 so it never gets squeezed */}
        <button
          type="submit"
          disabled={!input.trim() || loading}
          className="flex-shrink-0 w-10 h-10 bg-gradient-to-r from-blue-600 to-purple-600
            rounded-full flex items-center justify-center text-white hover:opacity-90
            transition disabled:opacity-40 disabled:cursor-not-allowed shadow"
        >
          {loading
